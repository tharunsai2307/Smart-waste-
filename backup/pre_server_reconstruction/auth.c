    ...
    while (1) {
        ...
        if (status == 1) {
            ...
            if (loggedInUser.role == ROLE_ADMIN) {
                handleAdminMenu();
            } else if (loggedInUser.role == ROLE_LOCAL_HUB_MANAGER || loggedInUser.role == ROLE_RECYCLING_MANAGER) {
                handleManagerMenu();
            } else if (loggedInUser.role == ROLE_CLEANER || loggedInUser.role == ROLE_DRIVER) {
                handleOperatorMenu();
