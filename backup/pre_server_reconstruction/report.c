...
void displayOverallDashboard() {
    ...
    if (fp) {
        ...
        while (fread(&c, sizeof(CollectionRequest), 1, fp)) {
            if (c.status == COLLECTION_REQUESTED || c.status == COLLECTION_UNDER_REVIEW) pendingReqs++;
            else if (c.status != COLLECTION_COMPLETED && c.status != COLLECTION_CANCELLED) activeReqs++;
        }
        fclose(fp);
    }
